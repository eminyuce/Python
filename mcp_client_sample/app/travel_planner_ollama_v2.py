...
def find_hotels_func(location: str, check_in: str, check_out: str) -> dict:
    """
...
def find_activities_func(location: str, date: str, preferences: str) -> dict:
    """
...
def find_transportation_func(location: str, origin: str, destination: str) -> dict:
    """
...
transportation_worker = Agent(
    ...
    verbose=True,
    allow_delegation=False,
    tools=[find_transportation_func],
    llm=llm,
    max_iter=1,
    max_retry_limit=3
)
...
flight_search_task = Task(
    description=dedent("""
        Use the Search for available flights between cities tool to find flight options from origin to destination on the specified date.
...
hotel_search_task = Task(
    description=dedent("""
        Use the Find available hotels in a location tool to search for accommodations in the destination for the specified check-in and check-out dates.
...
activity_planning_task = Task(
    description=dedent("""
        Use the Find available activities in a location tool to identify options in the destination for each day of the entire trip duration.
...
transportation_planning_task = Task(
    description=dedent("""
        Use the Find local transportation options tool to identify options at the destination for:
